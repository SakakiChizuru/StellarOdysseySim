...
class Battle {
	constructor({ player, mob, list_modifiers = null, verbose = false }) {
		...
		let weapon1 = player.weapon_ele1,
			weapon2 = player.weapon_ele2;
		if (weapon1 && (!weapon2 || weapon2 === "None") && weapon1 !== "None")
			weapon2 = weapon1;
		if (weapon2 && (!weapon1 || weapon1 === "None") && weapon2 !== "None")
			weapon1 = weapon2;
		let shield1 = player.shield_ele1,
			shield2 = player.shield_ele2;
		...
		this.credits_base =
			300.0 * (1.0 + 0.1 * mob.lvl) * (player.vip_status ? 1.1 : 1.0);
		this.exp_base = Math.floor(
			(20.0 + Math.floor(0.1 * mob.lvl)) * (player.vip_status ? 1.1 : 1.0)
		);
	}
	...
	do_one_attack(attacker, target) {
		...
		if (rng_attack < attacker_hit_chance) {
			...
			if (this.verbose) {
				console.log(
					`${attacker.name.value} attacks ${
						target.name.value
					} for ${curr_dmg.toFixed(2)} damage. ${
						target.name.value
					} is left with ${target.current_hp.toFixed(2)} HP.`
				);
			}
		} else if (this.verbose) {
			console.log(
				`${attacker.name.value} missed while attacking ${target.name.value}.`
			);
		}

		if (
			attacker instanceof Clone &&
			attacker.dual_shot_chance > 0.0 &&
			Math.random() < attacker.dual_shot_chance
		) {
			const rng_attack = Math.random();
			if (rng_attack < attacker_hit_chance) {
				...
				if (this.verbose) {
					console.log(
						`${attacker.name.value} DUAL attacks ${
							target.name.value
						} for ${curr_dmg.toFixed(2)} damage. ${
							target.name.value
						} is left with ${target.current_hp.toFixed(2)} HP.`
				...
				console.log(
					`${attacker.name.value} missed DUAL attack to ${target.name.value}.`
				);
			}
		}
	}
	...
	do_one_round() {
		...
		this.battle_is_over =
			this.mob.current_hp === 0.0 ||
			this.clone_squad.squad.every((clone) => clone.current_hp === 0.0);
	...
	get_revenue_print(win_chance = 1.0) {
		...
		return `Credits per hour: ${millify(
			revenue_per_hour
	...
	get_revenue(
		...
		const tot_credits =
			this.credits_base * (1.0 + income_boost + reputation);
